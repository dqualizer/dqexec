...
class K6MQConfig {
    @Bean
    @Qualifier("k6")
    fun k6Exchange(@Value("\${dqualizer.messaging.exchanges.k6.name}") k6Exchange: String?): TopicExchange {
        return TopicExchange(k6Exchange)
    }

    @Bean
    @Qualifier("k6")
    fun k6Queue(@Value("\${dqualizer.messaging.queues.k6.name}") k6Queue: String?): Queue {
