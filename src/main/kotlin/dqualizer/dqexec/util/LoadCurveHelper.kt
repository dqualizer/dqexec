...
/**
 * A helper class that calculates points on a load curve of the format y=t^e with e being the
 * exponent.
 *
...
class LoadCurveHelper(exponent: Double, ymin: Double, ymax: Double, targetDuration: Double) {
  ...
  /**
   * Returns the value for a given point in time for a load profile of given length "duration" and
   * with the form y=t^e with e being an exponent defined at initalization. At the beginning (time
   * = 0) the value is ymin and at the end (time = duration) the value is ymax.
   *
